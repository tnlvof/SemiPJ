...
public class AnalyseDao {
	...
	public HashMap<String, ArrayList<HashMap>> doAnalyse(String areaCode, String catagoryCode, Connection con) {
		HashMap<String, ArrayList<HashMap>> allTable = new HashMap<String, ArrayList<HashMap>>();
		ArrayList<HashMap> table = null;
		HashMap<String, String> row = null;
		
		String[] tableName = new String[]{"상권_지수지표", "상권_점포","상권배후지_점포","상권_추정매출","상권배후지_추정매출","상권_추정유동인구","배후지_추정유동인구","상권_상주인구","상권배후지_상주인구","상권_직장인구","상권배후지_직장인구","상권_아파트","상권배후지_아파트","상권_소득소비","상권배후지_소득소비","상권_집객시설","상권배후지_집객시설"};
		...
		String[] 배후지_추정유동인구 = new String[]{"기준_년월_코드","상권_코드","총_수","남성_수","여성_수","연령대_10_수","연령대_20_수","연령대_30_수","연령대_40_수","연령대_50_수","연령대_60_이상_수","시간대_1_수","시간대_2_수","시간대_3_수","시간대_4_수","시간대_5_수","시간대_6_수","월요일_수","화요일_수","수요일_수","목요일_수","금요일_수","토요일_수","일요일_수"};
		//상주인구 테이블 컬럼명
		...
		String[][] colsName = new String[][]{지표지수, 점포, 점포, 추정매출, 추정매출, 추정유동인구, 배후지_추정유동인구, 상주인구, 상주인구, 직장인구, 직장인구, 아파트, 아파트, 소득소비, 소득소비, 집객시설, 집객시설};
		...
		try {
			for (int i = 0; i < tableName.length; i++) {
				...
				}else if(tableName[i].contains("지수지표")||tableName[i].contains("추정매출")){
					pstmt = con.prepareStatement(query);
					pstmt.setString(1, areaCode);
					pstmt.setString(2, catagoryCode);	
				}else{
					pstmt = con.prepareStatement(query);
					pstmt.setString(1, areaCode);
				}
				...
				while(rset.next()){
					...
					if(!tableName[i].contains("점포")){
						for (int j = 0; j < colsName[i].length; j++) {						
							System.out.println(i + " " + j + " " + colsName[i][j] + " " +rset.getString(j+1));
							row.put(colsName[i][j], rset.getString(j+1));

							//System.out.println(colsName[i][j]);
						}
					}
